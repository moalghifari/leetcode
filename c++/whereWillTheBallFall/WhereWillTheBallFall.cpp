 ...
 * bottom after dropping the ball from the i^th column at the top, or -1 if the ball gets stuck in the 
 * box.
 * 
 * Example 1:
 * 
...
public:
    vector<int> findBall(vector<vector<int>>& grid) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            int jSecond;
            for (int i = 0; i < m; i++) {
                jSecond = jFirst + grid[i][jFirst];
                if (jSecond < 0 || jSecond >= n || grid[i][jFirst] != grid[i][jSecond]) {
                    jFirst = -1;
                    break;
                }
                jFirst = jSecond;
            }
            ans.push_back(jFirst);
        }
        return ans;
    }
};
