...
EXTENTION="$2"
FILETYPE="Java"
if [[ $EXTENTION == ".java" ]]; then
    FILETYPE="Java"
elif [[ $EXTENTION == ".cpp" ]]; then
    FILETYPE="C++"
elif [[ $EXTENTION == ".py" ]]; then
    FILETYPE="Python"
elif [[ $EXTENTION == ".sql" ]]; then
    FILETYPE="MySQL"
...
file=`${SCRIPT_PATH}/comments.sh ${leetcode_url} ${EXTENTION} | grep updated | awk '{print $1}'`
...
readme=`${SCRIPT_PATH}/readme.sh ${file} ${FILETYPE}`
