...
class Solution {
    public int deleteAndEarn(int[] nums) {
        int n = 10001;
        int[] mapValue = new int[n];
        for (int num: nums) {
            mapValue[num] += num;
        }
        int maxValueTwoBehind = 0;
        int maxValueOneBehind = mapValue[0];
        for (int i = 0; i < n; i++) {
            int temp = Math.max(maxValueTwoBehind + mapValue[i], maxValueOneBehind);
            maxValueTwoBehind = maxValueOneBehind;
            maxValueOneBehind = temp;
        }
        return maxValueOneBehind;
    }
}
